...
class Pathfinder:
    ...
    def create_path(self, start_point, end_point):
        ...
        for i in range(main_game.set_check()):
            ...
            self.grid.cleanup()
            main_game.path_print(path)
            print(i+1, 'is', len(path))
            for j in range(i):
                if len(path) <= len(self.path_list[j]):
                    for g in range(len(path)):
                        if path[g] == self.path_list[j][g]:
                            main_game.collision_marker(path[g])
                elif len(path) > len(self.path_list[j]):
                    for g in range(len(self.path_list[j])):
                        if path[g] == self.path_list[j][g]:
                            main_game.collision_marker(path[g])
                print(i+1, j+1)
